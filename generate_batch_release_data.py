...
def generate_batch_release_data(year: int):
    ...
    for _, batch in mfg_df.iterrows():
        ...
        records.append({
            ...
            "packaging_complete_date": (mfg_date + timedelta(days=random.randint(1, 3))).strftime("%Y-%m-%d"),
            ...
            "theoretical_tablets": int(batch.get("batch_size_kg", 200) * 1000 / 0.6),
            "actual_tablets": int(batch.get("batch_size_kg", 200) * 1000 / 0.6 * yield_value / 100),
            
            "has_deviation": "Yes" if has_deviation else "No",
            "deviation_reference": f"DEV-{year}-{random.randint(1, 500):04d}" if has_deviation else "",
            "deviation_classification": random.choice(["Major", "Minor"]) if has_deviation else "",
            "deviation_closed": "Yes" if has_deviation and disposition == "Released" else ("No" if has_deviation else ""),
            
            "has_oos": "Yes" if has_oos else "No",
            "oos_reference": f"OOS-{year}-{random.randint(1, 100):04d}" if has_oos else "",
            "oos_resolved": "Yes" if has_oos and disposition == "Released" else ("No" if has_oos else ""),
            
            "change_control_applicable": "Yes" if random.random() < 0.1 else "No",
            "change_control_reference": f"CC-{year}-{random.randint(1, 50):03d}" if random.random() < 0.1 else "",
            
            "checklist_batch_record": checklist_items.get("Batch record review complete", "Yes"),
            "checklist_ipc": checklist_items.get("All IPC results within specification", "Yes"),
            "checklist_qc_complete": checklist_items.get("QC testing complete and approved", "Yes"),
            "checklist_deviation": checklist_items.get("Deviation review complete", "Yes"),
            "checklist_change_control": checklist_items.get("Change control review complete", "Yes"),
            "checklist_environmental": checklist_items.get("Environmental monitoring acceptable", "Yes"),
            "checklist_calibration": checklist_items.get("Equipment calibration verified", "Yes"),
            "checklist_raw_materials": checklist_items.get("Raw material documentation verified", "Yes"),
            "checklist_stability": checklist_items.get("Stability protocol assigned", "Yes"),
            "checklist_label": checklist_items.get("Label reconciliation complete", "Yes"),
            "checklist_yield": checklist_items.get("Yield within acceptable limits", "Yes"),
            "checklist_packaging": checklist_items.get("Packaging integrity verified", "Yes"),
            
            "disposition": disposition,
            "disposition_date": release_date.strftime("%Y-%m-%d") if release_date else "",
            "hold_reason": "; ".join(hold_reasons) if hold_reasons else "",
            "rejection_reason": "Quality failure" if disposition == "Rejected" else "",
            
            "days_to_release": days_to_release,
            "expedited_release": "Yes" if days_to_release and days_to_release < 10 else "No",
            
            "expiry_date": expiry_date.strftime("%Y-%m-%d"),
            "shelf_life_months": 36,
            "retest_date": (mfg_date + timedelta(days=365*2)).strftime("%Y-%m-%d"),
            ...
            "stability_station": random.choice(["Long-term", "Accelerated", "Both"]),
            
            "warehouse_location": f"FG-{random.choice(['A', 'B', 'C'])}-{random.randint(1, 100):03d}",
            "shipped_quantity_pct": random.randint(0, 100) if disposition == "Released" else 0,
