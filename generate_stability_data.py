...
STABILITY_CONDITIONS = {
    "Long-term": {"temp_c": 25, "rh_percent": 60, "timepoints": [0, 3, 6, 9, 12, 18, 24, 36]},
...
def generate_stability_data(year: int):
    ...
    for _, batch_row in stability_batches.iterrows():
        ...
        for condition, params in STABILITY_CONDITIONS.items():
            ...
            baseline_total_impurities = round(baseline_impurity_a + np.random.exponential(0.08), 4)
            ...
            for timepoint in params["timepoints"]:
                test_date = mfg_date + timedelta(days=timepoint * 30)  # Approximate months
                ...
                assay = round(baseline_assay - assay_decrease + np.random.normal(0, 0.3), 2)
                ...
                dissolution = round(baseline_dissolution - diss_decrease + np.random.normal(0, 1), 1)
                ...
                total_impurities = round(baseline_total_impurities + impurity_increase * 2, 4)
                
                # Water content may increase
                water_increase = time_factor * np.random.uniform(0.1, 0.3) if condition == "Accelerated" else time_factor * np.random.uniform(0.02, 0.1)
                ...
                if assay < 95 or total_impurities > 0.4:
                    appearance = random.choice(["Slightly yellow tablets", "Discolored tablets"])
                else:
                ...
                impurity_result = "Pass" if impurity_a <= 0.10 and total_impurities <= 0.50 else "Fail"
                water_result = "Pass" if water_content <= 3.0 else "Fail"
                appearance_result = "Conforms" if "Discolored" not in appearance else "Does Not Conform"
                
                overall = "Pass" if all([assay_result == "Pass", dissolution_result == "Pass",
                                         impurity_result == "Pass", water_result == "Pass",
                                         appearance_result == "Conforms"]) else "Fail"
                ...
                records.append({
                    ...
                    "analyst_id": random.choice([f"QC-{i:03d}" for i in range(1, 21)]),
                    "reviewed_by": random.choice([f"QC-{i:03d}" for i in range(1, 5)]),
                    "comments": "" if overall == "Pass" else "OOS investigation initiated" if random.random() > 0.5 else "Trend monitoring required"
                })
    ...
    print(f"   ✓ Generated {len(df):,} stability test records for {num_stability_batches} batches")
    return df
