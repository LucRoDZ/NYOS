...
DEPARTMENTS = ["Manufacturing", "Quality Control", "Quality Assurance", "Warehouse", "Engineering", "Packaging"]
...
def generate_capa_data(year: int):
    ...
    for i in range(num_capas):
        ...
        source = np.random.choice(
            list(CAPA_SOURCES.keys()),
            p=list(CAPA_SOURCES.values())
        )
        ...
        # Root cause analysis
        rca_method = random.choice(["5-Why", "Fishbone Diagram", "Fault Tree Analysis", "FMEA", "Is/Is-Not Analysis"])
        ...
        if days_since_open > target_days + 30:
            status = random.choices(["Closed - Effective", "Closed - Not Effective", "Overdue"], weights=[0.7, 0.1, 0.2])[0]
        elif days_since_open > target_days:
            status = random.choices(["Closed - Effective", "In Progress", "Overdue"], weights=[0.5, 0.3, 0.2])[0]
        elif days_since_open > target_days * 0.5:
            status = random.choices(["Closed - Effective", "In Progress", "On Hold"], weights=[0.3, 0.6, 0.1])[0]
        else:
            status = random.choices(["In Progress", "Investigation", "Planning"], weights=[0.4, 0.3, 0.3])[0]
        
        # Completion dates based on status
        if "Closed" in status:
            actual_completion = capa_open_date + timedelta(days=random.randint(target_days - 15, target_days + 30))
        ...
        if status == "Closed - Effective":
            ...
            effectiveness_date = actual_completion + timedelta(days=random.randint(30, 90))
        ...
        records.append({
            ...
            "product_affected": "PARA-500-TAB" if random.random() > 0.2 else "Multiple Products",
            "batch_affected": f"PARA-{str(year)[-2:]}-{random.randint(1, 7000):04d}" if source in ["Deviation", "OOS Investigation"] else "N/A",
            ...
            "contributing_factors": random.choice(["Workload", "Shift change", "New personnel", "Equipment age", "None identified"]),
            ...
            "days_to_close": (actual_completion - capa_open_date).days if actual_completion else None,
            "days_late": days_late,
            ...
            "effectiveness_date": effectiveness_date.strftime("%Y-%m-%d") if effectiveness_date else "",
            "recurrence": recurrence,
            
            "linked_capas": f"CAPA-{year}-{random.randint(1, i+1):04d}" if random.random() < 0.1 and i > 0 else "",
            "regulatory_impact": "Yes" if risk_score == "Critical" else "No",
            "cost_estimate_usd": random.randint(500, 50000) if status != "Planning" else None,
            
            "created_by": f"QA-{random.randint(1, 15):02d}",
            "last_updated": (capa_open_date + timedelta(days=random.randint(1, 30))).strftime("%Y-%m-%d"),
            "comments": ""
        })
