...
def generate_extended_manufacturing_data(year: int):
    ...
    for day_offset in range(total_days):
        ...
        for _ in range(daily_batches):
            ...
            operator_secondary = random.choice([op for op in OPERATORS if op != operator_primary])
            ...
            if year == 2021 and current_month in [9, 10, 11] and tablet_press == "Press-A":
                ...
                compression_force_main_kn = round(np.random.normal(18.0 + force_increase, 1.5), 2)
            
            # 2025 Press-B drift (August 1-15)
            if year == 2025 and current_date >= datetime(2025, 8, 1) and current_date <= datetime(2025, 8, 15) and tablet_press == "Press-B":
                compression_force_main_kn = round(np.random.normal(22.0, 1.0), 2)
            
            # =====================================================
            # IN-PROCESS CONTROLS (IPC)
            # =====================================================
            ipc_weight_check_pass = "Pass" if abs(tablet_weight_mg - 500) < 25 else "Fail"
            ipc_hardness_check_pass = "Pass" if 100 <= tablet_hardness_n <= 150 else "Fail"
            ipc_thickness_check_pass = "Pass" if 4.2 <= tablet_thickness_mm <= 4.8 else "Fail"
            ipc_friability_check_pass = "Pass" if friability_percent < 1.0 else "Fail"
            ipc_disintegration_check_pass = "Pass" if disintegration_time_min < 15 else "Fail"
            
            # =====================================================
            # YIELD CALCULATIONS
            # =====================================================
            theoretical_yield_tablets = int((api_weight_kg * 1000) / 500 * 1000)  # 500mg tablets
            actual_yield_tablets = int(theoretical_yield_tablets * np.random.normal(0.985, 0.01))
            yield_percent = round((actual_yield_tablets / theoretical_yield_tablets) * 100, 2)
            yield_percent = max(90.0, min(100.0, yield_percent))
            
            # Rejects during compression
            reject_count = int(np.random.exponential(50))
            reject_reason = random.choice(["Weight", "Capping", "Sticking", "Chipping", "None"])
            if reject_count < 10:
                reject_reason = "None"
            ...
            # Downtime events
            downtime_minutes = round(np.random.exponential(15), 1) if random.random() < 0.2 else 0
            downtime_reason = random.choice(["Equipment adjustment", "Tool change", "Material shortage", "Cleaning"]) if downtime_minutes > 0 else "None"
    ...
    print(f"   ✓ Generated {len(df):,} extended manufacturing records ({len(df.columns)} columns)")
    return df
