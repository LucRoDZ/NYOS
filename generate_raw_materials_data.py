...
SUPPLIERS = {
    "SUP-001": {"name": "PharmaChem Industries", "country": "India", "materials": ["RM-API-001"], "tier": "Strategic"},
    "SUP-002": {"name": "ChemPure Ltd", "country": "China", "materials": ["RM-API-001"], "tier": "Approved"},
    "SUP-003": {"name": "ExcipientCo", "country": "Germany", "materials": ["RM-EXC-001", "RM-EXC-003"], "tier": "Strategic"},
    "SUP-004": {"name": "PolymerTech", "country": "USA", "materials": ["RM-EXC-002"], "tier": "Approved"},
    "SUP-005": {"name": "LubeSpec Inc", "country": "Japan", "materials": ["RM-EXC-004", "RM-EXC-005"], "tier": "Approved"},
    "SUP-006": {"name": "PackWell Solutions", "country": "USA", "materials": ["PM-PRI-001", "PM-PRI-002"], "tier": "Strategic"},
    "SUP-007": {"name": "PrintPack Ltd", "country": "UK", "materials": ["PM-SEC-001", "PM-LBL-001"], "tier": "Approved"},
    "SUP-008": {"name": "API Global", "country": "India", "materials": ["RM-API-001"], "tier": "Backup"}  # New supplier in 2025
}
    ...
    for month in range(1, 13):
        ...
        for mat_code, mat_info in MATERIALS.items():
            ...
            for _ in range(num_receipts):
                ...
                # Select supplier
                available_suppliers = [s for s, info in SUPPLIERS.items() if mat_code in info["materials"]]
                ...
                if mat_code == "RM-API-001" and year == 2025 and month >= 11:
                    ...
                    supplier_id = random.choice(available_suppliers) if available_suppliers else "SUP-001"
                ...
                # Supplier lot/batch
                supplier_lot = f"{supplier_id[-3:]}-{year}{month:02d}-{random.randint(1, 99):02d}"
                
                # COA data
                coa_received = "Yes"
                coa_matches_spec = random.choices(["Yes", "No"], weights=[0.98, 0.02])[0]
                
                # Hidden scenario: 2022 excipient supplier issue
                if year == 2022 and month in [5, 6, 7] and mat_code in ["RM-EXC-001", "RM-EXC-003"] and supplier_id == "SUP-003":
                ...
                # Test results
                test_status = "Pass" if coa_matches_spec == "Yes" else random.choice(["Pass", "Fail", "Conditional"])
                ...
                if "API" in mat_code:
                    ...
                    if supplier_id == "SUP-008":
                        assay_result = round(np.random.normal(99.2, 0.5), 2)  # Slightly lower, more variable
                    assay_spec = "98.0-102.0%"
                    
                    impurity_total = round(np.random.normal(0.15, 0.03), 3)
                    if supplier_id == "SUP-008":
                        impurity_total = round(np.random.normal(0.22, 0.05), 3)  # Higher impurities
                    impurity_spec = "≤ 0.5%"
                    ...
                    identity_test = "Pass"
                    assay_result = round(np.random.normal(99.0, 0.5), 2) if mat_code == "RM-EXC-002" else None
                    assay_spec = "98.0-102.0%" if assay_result else "N/A"
                    
                    impurity_total = None
                    impurity_spec = "N/A"
                    
                    moisture = round(np.random.normal(4.0, 0.5), 2) if "MCC" in mat_info["name"] else round(np.random.normal(5.0, 0.5), 2)
                    moisture_spec = "≤ 5.0%" if "MCC" in mat_info["name"] else "≤ 7.0%"
                    
                    particle_size_d50 = round(np.random.normal(100, 10), 1) if "MCC" in mat_info["name"] else None
                ...
                if test_status == "Pass":
                    ...
                    disposition_date = receipt_date + timedelta(days=random.randint(3, 7))
                elif test_status == "Conditional":
                    disposition = random.choice(["Released with Deviation", "Pending QA Review"])
                    disposition_date = receipt_date + timedelta(days=random.randint(7, 14))
                else:
                    disposition = "Rejected"
                    disposition_date = receipt_date + timedelta(days=random.randint(5, 10))
                
                # Retest date calculation
                manufacturing_date = receipt_date - timedelta(days=random.randint(30, 180))
                retest_date = manufacturing_date + timedelta(days=mat_info["retest_months"] * 30)
                ...
                records.append({
                    ...
                    "deviation_raised": "Yes" if disposition == "Released with Deviation" else "No",
                    "deviation_reference": f"DEV-{year}-{random.randint(1, 500):04d}" if disposition == "Released with Deviation" else "",
...
def generate_supplier_performance(year: int):
    ...
    for supplier_id, supplier_info in SUPPLIERS.items():
        ...
        records.append({
            ...
            "quality_acceptance_rate_pct": round((num_deliveries - quality_rejections) / num_deliveries * 100, 1),
            ...
            "audit_classification": "Approved" if audit_score >= 80 else "Conditional" if audit_score >= 70 else "Not Approved",
            "open_capas": random.randint(0, 3),
            
            "qualification_status": "Qualified" if audit_score >= 70 else "Under Review",
            "qualification_expiry": (last_audit_date + timedelta(days=365)).strftime("%Y-%m-%d"),
            ...
            "overall_rating": "A" if audit_score >= 90 else "B" if audit_score >= 80 else "C" if audit_score >= 70 else "D",
            "comments": ""
        })
...
if __name__ == "__main__":
    ...
    if all_sp:
        ...
        print(f"✅ Combined file: supplier_performance_ALL.csv ({len(combined_sp):,} records)")
