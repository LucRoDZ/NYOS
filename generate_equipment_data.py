...
EQUIPMENT = {
    ...
    "GR-001": {
        ...
        "pm_freq_months": 3,
        "parameters": ["Impeller Speed", "Chopper Speed", "Load Cell", "Temperature Sensor"]
    },
    "GR-002": {
        "name": "High Shear Granulator HSG-500",
        "type": "Granulator",
    ...
    "DR-001": {
        ...
        "calibration_freq_months": 6,
        "pm_freq_months": 3,
        "parameters": ["Inlet Air Temperature", "Outlet Air Temperature", "Air Flow", "Product Temperature"]
    },
    "DR-002": {
        "name": "Fluid Bed Dryer FBD-200",
        "type": "Dryer",
    ...
    "PRESS-A": {
        ...
        "pm_freq_months": 1,
        "parameters": ["Pre-compression Force", "Main Compression Force", "Turret Speed", "Weight Control System", "Hardness Monitor"]
    },
    "PRESS-B": {
        "name": "Rotary Tablet Press RTP-45",
        "type": "Tablet Press",
    ...
    "HPLC-001": {
        ...
        "calibration_freq_months": 12,
        "pm_freq_months": 6,
        "parameters": ["Pump Flow Rate", "UV Detector Wavelength", "Column Oven Temperature", "Autosampler Precision"]
    },
    "HPLC-002": {
    ...
    "DISS-001": {
        ...
        "calibration_freq_months": 6,
        "pm_freq_months": 3,
        "parameters": ["Paddle Speed", "Temperature", "Vessel Position", "Sampling Probe Position"]
    },
    "DISS-002": {
        "name": "Dissolution Apparatus USP-II",
        "type": "Dissolution Tester",
...
def generate_calibration_records(year: int):
    ...
    for eq_id, eq_info in EQUIPMENT.items():
        ...
        for cal_num in range(num_calibrations):
            ...
            month = (cal_num * eq_info["calibration_freq_months"]) + random.randint(1, eq_info["calibration_freq_months"])
            month = min(month, 12)
            ...
            for param in eq_info["parameters"]:
                ...
                records.append({
                    ...
                    "next_due_date": (actual_date + timedelta(days=eq_info["calibration_freq_months"]*30)).strftime("%Y-%m-%d"),
                    ...
                    "standard_traceable": "NIST" if eq_info["criticality"] == "Critical" else "Manufacturer",
                    ...
                    "oot_investigation": f"OOT-{year}-{random.randint(1, 50):03d}" if result == "Fail" else "",
...
def generate_maintenance_records(year: int):
    ...
    for eq_id, eq_info in EQUIPMENT.items():
        ...
        for pm_num in range(num_pms):
            ...
            # Parts replaced
            parts_replaced = random.choices(["None", "Seals", "Filters", "Bearings", "Belts", "Multiple"], 
                                           weights=[0.5, 0.2, 0.15, 0.08, 0.05, 0.02])[0]
            
            # PM result
            pm_result = random.choices(["Pass", "Pass with observations", "Fail - repair required"], 
            ...
            records.append({
                ...
                "scheduled_date": scheduled_date.strftime("%Y-%m-%d"),
                "actual_date": actual_date.strftime("%Y-%m-%d"),
                "next_due_date": (actual_date + timedelta(days=eq_info["pm_freq_months"]*30)).strftime("%Y-%m-%d"),
                
                "activities_performed": "; ".join(selected_activities),
                "parts_replaced": parts_replaced,
                "parts_cost_usd": random.randint(0, 2000) if parts_replaced != "None" else 0,
                
                "result": pm_result,
                "observations": f"Minor {random.choice(['wear', 'corrosion', 'buildup'])} observed" if "observations" in pm_result else "",
                "follow_up_required": "Yes" if pm_result != "Pass" else "No",
                "follow_up_ref": f"WO-{year}-{random.randint(1, 500):04d}" if pm_result != "Pass" else "",
...
if __name__ == "__main__":
    ...
    if all_pm:
        ...
        print(f"✅ Combined file: preventive_maintenance_ALL.csv ({len(combined_pm):,} records)")
